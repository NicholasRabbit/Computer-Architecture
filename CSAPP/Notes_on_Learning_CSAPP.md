...
Why can a PIPE- guarantee that in the decode stage of `0x00c addl %ead, %eax` it can generate the correct value of `valB`, namely the value of `%eax`  from the ALU in the previous "execute stage" of `0x006 irmovl $3, %eax`? 

The reason is that the `valB` needed by the end of "Decode Stage" of `0x00c` will be generated at the "Execute Stage" of `0x006`. This value has been generated by ALU and is ready to be passed to the next pipeline register when the next cycle clock rises. Hence, it is valid for the "Execute Stage" of `0x00c`.

##### 4.5.8 Load/Use Data Hazards

(1)  Elaboration of Figure 4.53

What causes the hazards in cycle 7? 

In `0x18: mrmovl 0(%edx), %eax`, when clock rises in cycle 7, it is in the "Execute Stage" and the `valM` of `0(%edx)` hasn't been fetched from memory, therefore, the "Decode Stage" obtain invalid data when it is forwarding when the next cycle clock rises for the next "Execute State". 
