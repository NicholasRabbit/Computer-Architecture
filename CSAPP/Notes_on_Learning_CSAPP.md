...
# 3, hello.o
gcc -c hello.s -o hello.o  # "hello.o" is a relocatable object program.
# Or
...
1. `hello.o` is an relocatable object program/file. It is generated by a compiler before the linked process. After a compiler linking it with other object files/programs, an executable object program will be produced. 
...
Why can a PIPE- guarantee that in the decode stage of `0x00c addl %edx, %eax` it can generate the correct value of `valB`, namely the value of `%eax`  from the ALU in the previous "execute stage" of `0x006 irmovl $3, %eax`? 
...
(2) Since in a pipelined process, only interlocks reduce the throughput, it is not so significant that we can achieve a goal of high performance. (Explanation of the last sentence of this chapter. ) 
